...
const GatewayLayer = ({ mapRef, mapRenderKey }) => {
    ...
    // 1. Locate the Gateway Layer
    let gwGroup = g.select('.gateway-layer');
    
    if (gwGroup.empty()) {
        // 2. Find the first star system (any rect that isn't the background 'rect1')
        const firstSystemNode = g.select('rect:not(#rect1)').node();
        
        if (firstSystemNode) {
        // 3. Insert the group BEFORE the first star system to put it "behind" them
        gwGroup = g.insert('g', () => firstSystemNode)
                    .attr('class', 'gateway-layer');
        } else {
        // Fallback to append if the map isn't fully ready
        ...
        linkItem.append('line')
            ...
            pair.links.forEach(l => {
                ...
                <div class="gateway-col">
                    ...
                    <div class="gw-info">Volume: ${l.MaxShipVolume || 1} m³</div>
